...
fn main() {
    ...
    for cap in re.captures_iter(&file) {
        ...
        let p = Vector2::new(p_r.0+10000000000000.0, p_r.1+10000000000000.0);
        let m = Matrix2::from_columns(&[a, b]);

        //my input does not have collinear a and b
        let i = m.try_inverse().unwrap();
        let presses = i*p;
        if is_integer(presses[0]) && is_integer(presses[1]){
...
fn is_integer(value:f64) -> bool{
    ...
    (value - value.round()).abs() < (a.powf(16.0)*f32::EPSILON).into()
}